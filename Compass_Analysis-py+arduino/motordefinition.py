...
# Read data
static = pd.read_csv(nameyboi+'.csv')

# format
static, s_x_max, s_x_min, s_y_max, s_y_min, s_h_max, s_h_min = format_data(static)

# Plotting
vary_plot(static, s_x_max, s_x_min, s_y_max, s_y_min, s_h_max, s_h_min, nameyboi)

#plt.show()

# Returning Key stats
motoroff = '+/- '+str(round(abs(s_h_max[0]-s_h_min[0])/2,3))+' rads'
motoroffd = '+/- '+str(round(abs(s_h_max[0]-s_h_min[0])*180/(2*math.pi),3))+' degrees'

print('Compass Accuracy '+nameyboi+':')
print(motoroff)
print(motoroffd)
